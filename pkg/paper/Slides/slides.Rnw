...
\begin{itemize}
\item Actually some alternatives
	\begin{itemize}
	\item ARIMA
	\item GARCH
...
We can employ a Functional Central Limit Theorem (FCLT) on $efp(\cdot)$
\end{frame}
...
\sum_{b=1}^B \sum_{t=\tau_{b-1} + 1}^{\tau_b} loglik (y_t | \theta^{(b)}, \sigma^{(b)}, \delta^{(b)}) \nonumber
\end{eqnarray}
...
% doppelmax given by: max_i=1...n max_j=1...k |efp_j(i/n))| ; i.e. aggregated over both time and components
\end{frame}
...
%\end{frame}
\begin{frame}
\frametitle{Breakpoint Selection}
...
\begin{center}
<<ireland, fig=TRUE, height=5, width=8>>=
...
% does not work --> why????
%\begin{frame}
%\frametitle{GL--right and left skewed}
%
%
%\begin{center}
%<<glrl, fig=TRUE, height=4, width=8>>=
%df <- data.frame(glogisr = rglogis(100000, 1,2,1.5), glogisl = rglogis(100000, 1,2,.7))
%df.m <- melt(df)
