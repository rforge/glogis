...
3--dimensional parameter $\phi = (\theta, \sigma, \delta)$:
...
Empirical scores $s(y_t|\hat{\phi})$ to examine changes
...
The empirical fluctuation process $efp(\cdot)$:
...
Functional Central Limit Theorem (FCLT) for $efp(\cdot)$
\end{frame}
...
\item Estimate models with B breakpoints; select best via modified BIC 
