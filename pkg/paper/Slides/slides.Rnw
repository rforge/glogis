...
<<plot1d, eval=FALSE>>=
plot(x, main = "HICP", ylab = "100 * diff(log(HICP))", xlab = "Time")
@
...
% differences still have to remain \cite{duarte}, ECB stabilises the global inflation rate, no more power to focus on some nation
  \item 21 monthly unadjusted Harmonised Index of Consumer Prices (HICP) series
...
\item Our model
	...
	\item Not interested in correlation structure
...
Assuming $\phi = (\theta, \sigma, \delta)$ is constant:
...
Use empirical scores $s(y_t|\hat{\phi})$ for testing
...
3 components $(s_{\theta}, s_{\sigma}, s_{\delta})$, with $\tilde{y} = \exp^{-\frac{y-\theta}{\sigma}}$
...
\begin{eqnarray}
efp(t) = \hat{V}^{-1/2} n^{-1/2} \sum_{i=1}^{\lfloor nt \rfloor} s(y_i|\hat{\theta}, \hat{\sigma}, \hat{\delta}) \; (0 \leq t \leq 1), \nonumber \\
...
\begin{itemize}
\item Functional Central Limit Theorem (FCLT) for $efp(\cdot)$
\item $W^0$ converges to a 3--dimensional Brownian Bridge
\end{itemize}
\end{frame}
...
\begin{itemize}
\item If instability detected, estimate B breakpoints $\tau_1,...,\tau_B$ via maximization of full segmented likelihood:
...
\sum_{b=1}^{B+1} \sum_{t=\tau_b + 1}^{\tau_b} \log f(y_t | \theta^{(b)}, \sigma^{(b)}, \delta^{(b)}) \nonumber
\end{eqnarray}




\item Estimate models with $B=1,...6$ breakpoints; select best via modified BIC 
...
\hline
\textcolor{white}{Country} & \textcolor{white}{Segment} & \textcolor{white}{Mean} & \textcolor{white}{Variance} & \textcolor{white}{Skewness}  \\
\hline
