...
\begin{itemize}
	\item 3 groups
		\begin{itemize}
		\item EURO countries
		\item EU members withouth ERM~II
...
\frametitle{Generalized Logistic Distribution (GL)}
...
% before starting with scores give motivation:
...
3 components $s()=(s_{\theta}, s_{\sigma}, s_{\delta})$:
...
For the 3--dimensional parameter $\phi = (\theta, \sigma, \delta)$ we test:

% under parameter stability
\begin{eqnarray}
& H_0: \phi_i = \phi_0 \; (i=1,...,n) \nonumber
\end{eqnarray}

% parameters estimated by ML 

\begin{eqnarray}
& \underset{\phi \in \Phi}{argmin} \sum_{t=1}^n \log f(y_t|\phi) = \hat{\phi}, \nonumber\\
& \sum_{t=1}^n s(y_t|\hat{\phi}) = 0 \nonumber
\end{eqnarray}

To examine changes we employ the empirical scores $s(y_t|\hat{\phi})$
...
The empirical fluctuation process $efp(\cdot)$ is given by:
...
We can employ a Functional Central Limit Theorem (FCLT) on $epf(\cdot)$
\end{frame}
...
%5. choose m with the best LWZ value (which is a modified BIC) 0.299*log(n)^2.1 (p.1701 Z(2010))
...
\begin{itemize}
\item If instability detected, estimate $B\in[1,6]$ breakpoints $\tau_1,...,\tau_B$ via maximization of full segmented likelihood:
...
\item Estimate models with B=1,...,6 breakpoints; select via modified BIC 
...
<<fig=TRUE, height=5.5, width=8>>=
<<plot4>>
@
% careful: the red lines are the doppelmax statistics not the supLM (just dont mention it:) 
% doppelmax given by: max_i=1...n max_j=1...k |epf_j(i/n))| ; i.e. aggregated over both time and components
\end{frame}
...
Some countries exhibit very similar patterns
...
% extreme skewness in case of Slovenia
...
\begin{center}
<<belga, fig=TRUE, height=3, width=4>>=
print(xyplot(cbind(100*diff(log(na.omit(HICP[, "Belgium"]))), 100*diff(log(na.omit(HICP[, "Luxembourg"])))), xlab="Time", ylab="", main="", screens=list("Belgium", "Luxembourg")))
@
\end{center}
\end{frame}
...
%xyplot(cbind(100*diff(log(na.omit(HICP[, "Belgium"]))), 100*diff(log(na.omit(HICP[, "Luxembourg"])))), xlab="Time", ylab="", main="", screens=list("Belgium", "Luxembourg"))
