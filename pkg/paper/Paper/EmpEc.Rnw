...
critical value at 5\% level (horizontal gray line), and estimated breakpoint (vertical dashed line).}
...
fitted mean (horizontal blue lines in the left panel of Figure~\ref{fig:slovenia1}) shows that
...
gray_red <- rgb(c(0.2, 0.0), c(0.2, 0.0), c(0.2, 0.0), alpha = c(0.3, 0.5)) ## actually gray-darkgray now
