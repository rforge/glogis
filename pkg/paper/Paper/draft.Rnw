...
The GL distribution in the parameterization used here is defined in \citet{Abberger:2002}
...
$\Sexpr{round(eu_mom[2, 1], digits = 3)}$ (\Sexpr{eu_breaks[1] + 1/12}--\Sexpr{eu_breaks[2]}) with a slight decrease to
