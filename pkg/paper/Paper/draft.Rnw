...
\author{\hfill Thomas Windberger$^a$ \hfill Achim Zeileis$^b$\thanks{Correspondence
to: Achim Zeileis, Department of Statistics,  Universit\"at Innsbruck,
Universit\"atsstra{\ss}e 15, 6020 Innsbruck, Austria. Tel: +43/512/507-7103. Fax: +43/512/507-2851.
E-mail: \email{Achim.Zeileis@R-project.org}} \hfill \hfill \\
{\small \it $^a$ Department of Statistics, Universit\"at Innsbruck, Austria} \\
{\small \it $^b$ Department of Statistics, Universit\"at Innsbruck, Austria}}
\Plainauthor{Thomas Windberger, Achim Zeileis}

\renewcommand{\abstractname}{Summary}
\Abstract{
	In an attempt to assess the effects of the EMU on inflation rate dynamics of its member states, this paper tests for structural breaks in the inflation dynamics of 21 European   countries. To capture   changes in the first three moments of the inflation rates, a model is adopted that makes use of the generalized logistic distribution. 
	There are   breaks in all but 4 countries.  Also, the differences between the inflation dynamics and its changes  are considerable. 
  Overall, there is a convergence towards a lower inflation rate, but with an increase in variance and a decrease in skewness.
  %% avoid "we"-constructs
  %% "adopt" better than "assume"
}
...
\fixme{affiliation should appear on title page}
Since the beginning of the European Union, the topic of a common currency
...
an ongoing debate. This concerns the issue of short--run and steady--state
inflation uncertainty -- in context to inflation expectations -- dealt with in
\citet{cap}, or the degree of similarity of short--run dynamic properties of the inflation rates in EU
...
Throughout the literature, there is still a
considerable degree of uncertainty as to what extent the introduction of the
euro, or monetary unions in general, affect the inflation rate. 

Basically, the way towards the EMU basically consisted of three stages: stage~I
...
\readme{problem of breaks addressed} The possible influence of these different stages on the inflation rate dynamics
suggested itself for a more thorough investigation. 
 
To inquire into possible changes in the dynamics of the inflation rate following
these stages, which may have led to considerable alterations, seems a promising task. 

However, a deeper inquiry into the effects of monetary unions on the inflation rates of its member
countries is in need of a method able to
capture changes in inflation dynamics. We contribute to this field of research by
developing a new method for testing for structural breaks in the dynamics of the
inflation rates. The framework utilized in this paper allows testing for any 
possible alternations  and thus helps interpreting 
...
distribution (GL). For this purpose, we build on previous research by \citet{Zeileis+Hornik:2007}. 

This \fixme{describe method} framework allows us to assess the evolution of mean, variance, and skewness over time.
Our estimation technique enables the simultaneous estimation of breaks in these moments. 
The skewness parameter turns out to be quite important to measure alterations in the
...
%Overall, we find four periods during which structural breaks
%happened. Two of them in the 1990s (early and late), another one in the early 2000s, and the last period covers the financial crisis. There is also a 
%tendency towards a lower mean inflation rate, combined with a decreasing difference between
%individual inflation rates.

\readme{more about aggregation...} Overall, we find that the countries of the core euro--area (as defined in Section~\ref{sec:results}) did not change after the second stage and ended up with a low mean and variance. The southern euro--area member states and Ireland decreased their inflation rates almost to a level of the core countries but with a higher variance and a non--symmetric skewness. The Eastern European countries did reduce mean and variance significantly, especially the two countries that joined the euro--zone first. In the case of the Nordic non--euro countries, there is either no change at all or a clear decline in mean and variance after overcoming national crisis. Generally, inflation rates, which were very different in the beginning of the sample, are becoming more similar in terms of all three moments.
...
PIIGS  (\Sexpr{paste(colnames(hicps)[piigs], collapse = ", ")}), and the
other countries (\Sexpr{paste(colnames(hicps)[others], collapse = ", ")}).
...
Finally, the inflation rates for the whole euro area\footnote{This is the euro--area 17 countries series as provided by the \citet{oecd} and covers the total euro--area. This series is calculated as a weighted average of the HICPs of now seventeen euro countries and starts in January 1996.}, are considered and three different regimes are found: (1)~until 
1999 with both a low mean and variance, (2)~the third and final stage of monetary integration,
accompanied with a significant increase in the mean, (3)~a period encompassing
the overheating of the economy that culminated in the financial crisis, showing a big increase
in variance and a lower mean inflation.

\readme{perhaps skip?} One clearly visible trend is the rise in inflation volatility in some of the euro countries (Austria, Greece, Italy, Luxembourg and Spain) and the  series for the whole euro area. \citet{emerson} emphasize  the fact that a high inflation rate is also more
variable and uncertain and thus causes more relative price variability, leading
to a less efficient price mechanism. From an economic point of view, a very volatile inflation rate will 
likely  contribute to greater macroeconomic instability. In the literature, there is no consensus about the effects of higher 
inflation volatility on the inflation rate. \citet{grier} find no evidence 
that higher inflation uncertainty raises the average inflation rate -- at least in the case of the USA. However,
\cite{jaro} finds a positive correlation between the level and the standard deviation of inflation. Naturally, an
efficient price mechanism will benefit from inflation rates that do not diverge too much from the expectations.

\fixme{some concluding remarks about the EU level bridging the current gap to the next subsection}
In any case, the EU series as a whole -- which is already an aggregate -- does sum up the general trend, namely the low inflation rates during the late 90s, a higher level until the financial crisis and a lower mean inflation rate thereafter. However, this series is too highly aggregated and starts too late to be able to draw precise conclusions regarding the convergence issue.  



%There is an increase in mean from
...
%$\Sexpr{round3(eu_mom[3, 1])}$ (\Sexpr{eu_breaks[2] + 1/12}--\Sexpr{end(eu)}) shortly before the financial crises.
%
...
In order to both aggregate the data to a number of sub--groups but avoid too high a level of compression, as is the case of the EU series, 
the countries are are grouped into the ``core'' of the eurozone, the PIIGS countries  and all the others. For this, we took account of possible
breaks in the moments. These groups are chosen so as to reflect the general dynamics of European inflation rates during the time period of interest.

The result of this is presented in Table~\ref{tab:results2} that shows lower quartile,
median, and upper quartile (25\%, 50\%, 75\% percentiles) of the first three moments of the
inflation series in January \Sexpr{yrs[1]}, \Sexpr{yrs[2]}, \Sexpr{yrs[3]} and \Sexpr{yrs[4]}. 

\fixme{also ``zoom out'' more, a big picture needs to be established}
The time periods are chosen according to four fundamental change periods. Namely, the early 1990s, with breaks in
Austria, Finland, France, Sweden, and UK. The late 1990s, with breaks in Greece, Italy, Luxembourg, Portugal, Czech Republic,
Estonia, and Hungary. The period after the second phase of convergence, with breaks in the early 2000s (Poland, Slovakia, and Slovenia)
and the last period consisting of two countries with the most visible effect of the financial crisis (Ireland and Spain). 

In the core--zone, there is a decrease in the lower quartile from \Sexpr{yrs[1]} to
\Sexpr{yrs[2]}. However, the interquartile range decreased by almost 20\%. The variance increased
slightly, but also with a 15\% decrease in interquartile range. The skewness
stayed symmetric in the median, with almost an 8\% increase in the interquartile range.
\readme{overview}Generally, there are no dramatical changes in the core--zone and after the initial stages, none of the core--countries
had another break. This reflects the fact that their respective monetary policies were already closely connected.

The PIIGS countries significantly decreased the median inflation rates from \Sexpr{yrs[1]} to \Sexpr{yrs[4]}. 
Whereas the interquartile range was very tight in \Sexpr{yrs[1]}, it increased in \Sexpr{yrs[2]} and again in \Sexpr{yrs[4]}.
However, the median values are much smaller. The variance increased from \Sexpr{yrs[1]} to \Sexpr{yrs[2]} but did not change much 
in \Sexpr{yrs[4]}. The interquartile range did not change much too. Median skewness changed considerable 
from highly positive to negative values. 
\readme{overview, skewness effects} The decline in skewness and the narrowing of the interquartile range reflects the
fact that significant shocks to the inflation rates, which for some series were quite common in the past, have become something of a rarity. 
Still, the PIIGS countries tend to come closer towards the core--countries on behalf of the general trend of lower values and 
narrower bands. 

The other countries were successful in decreasing all three moments from \Sexpr{yrs[1]} to \Sexpr{yrs[3]}. Still, their median
inflation rate is higher than those of the core countries. Also, the interquartile range of the mean decreased considerably. 
Contrary to the euro--zone and the PIIGS countries, variance decreased also, both in terms of the median value and the
interquartile range. The skewness decreased as well, as did the interquartile range of the skewness parameter.
\readme{overview} The other countries started with higher mean inflation values -- since some of them are Eastern European countries -- and 
effectually decreased the median inflation to a level much more similar to the core--zone than in \Sexpr{yrs[1]}. The bulk of changes 
in these countries took effect after \Sexpr{yrs[2]}, as a result of both an ending of the transitionary phase (from planned to free market economies) and the intended entry
into the EMU. 

In context of the convergence issue, we find that, with the interquartile range of the mean of all the aggregated zones declining and getting tighter, there is  
evidence of a convergence towards some common mean inflation rate in the countries of our sample, both within the subgroups and the three groups put together.
Regarding the variance, the PIIGS countries, and the core countries as well, saw an increase over time, which is mostly due to the influence of the financial
crisis with its ``up--down--up'' turn (as depicted in Figure~\ref{fig:glmean}) leading to a higher variance.
...
To assess changes in the dynamics of the inflation rates of a number of European countries both within
and without the EMU, a new method for estimating structural breaks resting on the 
assumption that the inflation rates are GL distributed has been  developed.
...
distribution might lead to potentially misleading conclusions concerning the dynamics. 
The decline in size and frequency of mostly positive outliers in many inflation rate series 
would have gone unnoticed without the skewness parameter to capture these changes. 

At the beginning of the sample, many countries did have very high inflation rates, especially the Eastern and Southern countries. 
For the core--zone, convergence was already achieved in \Sexpr{yrs[2]}. The PIIGS countries lowered their inflation rates considerably in preparation 
to the adoption of the euro. Over the time --  however -- many inflation rates declined considerably and are now much more in line with the inflation rates of the traditional low inflation countries of the core--euro zone. This is reflected especially well in the Eastern European countries, that lowered their high initial inflation level in the late 90s or prior to the entry into the ERM~II. They clearly benefited from the monetary discipline that is demanded for those countries who want to join the EMU. 
