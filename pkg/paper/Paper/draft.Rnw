...
adjusted returns $y_t$ ($t = 1, \dots, n$) are assumed to come from a
generalized logistic (GL) distribution \citep[see][]{johnson} with
three-dimensional parameter $\phi_t = (\theta_t, \sigma_t, \delta_t)^\top$ (for
location~$\theta_t$, scale~$\sigma_t$, and shape~$\delta_t$ at time $t$). Structural change
techniques are then employed to check whether the parameter vector $\phi_t$
...
\fixme{Z: potentially say something about the subsequent subsections}
