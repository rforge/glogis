...
## plot convergence in mean
  ...
  ## rolling GL means
  ## without Estonia,  Slovakia, Slovenia
  hicp_glmean <- rollapply(hicp, 12, glmean)
...
%(...some problems with graph creation inside .rnw, hope to fix it later ...) \\
% was i zum sega ket hob ba busetti --> vgl. archivierte Datei vom 16.11
% actually too detailed for so small a picture
% des if un die } for am ät dazuatoa

\begin{figure}[h!tp]
<<greyplot, fig=TRUE, height=8, width=8>>=
##if(!file.exists(file.path(root_dir, "Draft", "draft-greyplot.pdf"))) {
<<plot3>>
##}
@
...
%\begin{figure}[h!tp]
%\includegraphics[width=150mm, height=100mm]{draft-greyplot}
%\caption{Inflation rates and mean inflation rates over time}
%\end{figure}
...
%\begin{figure}[h!tp]
%\includegraphics[width=150mm, height=100mm]{diffmean}
%\caption{Differences in Euro--are mean without Ireland}
%\end{figure}

\begin{figure}[h!tp]
<<diffmean, fig=TRUE, height=8, width=8>>=
##if(!file.exists(file.path(root_dir, "Draft", "diffmean.pdf"))) {
<<plot4>>
##}
@
