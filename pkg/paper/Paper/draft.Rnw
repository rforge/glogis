...
\Abstract{
  ...
  a generalized logistic distribution. We find breaks in all but 4 countries.
  Overall, there is a convergence towards a lower inflation rate, but with an increase in variance and a change
  towards negative skewness.
  %% avoid "we"-constructs
...
\fixme{comment on results here} Overall, we find four periods during which structural breaks
...
Finally, the inflation rates for the whole euro area \fixme{Is EU euro area or European union?},
as provided by the OECD, are considered and three different regimes are found: (1)~until 
...
Table~\ref{tab:results2} presents lower quartile,
...
into the ``core'' of the eurozone, the PIIGS countries  and all the others. For this, we took account of possible
...
Estonia, and Hungary. And also the period after the second phase of convergence, with breaks in the early 2000s (Poland, Slovakia, and Slovenia)
and the two countries with the most visible effect of the financial crisis (Ireland and Spain). 
...
from highly positive to negative values. This is mostly due to the changes associated with the financial crisis, that was
responsible for deflationary shocks. The interquartile range of skewness decreased considerably.
...
This is also visible in the median values, that  diverge much less than at the beginning in \Sexpr{yrs[1]}.
...
crisis of 2008/9 did not have a significant effect on the inflation dynamics
...
More specifically, the mean inflation of the euro area \fixme{see above} from a GL fit in the 
...
\readme{start with 1) short recap of what we did, 2) conclusions}

To assess changes in the dynamics of the inflation rates of a number of European countries both within
and without the EMU, we developed a new method for estimating structural breaks resting on the 
assumption that the inflation rates are GL distributed.
...
The results of \citet{holte}, whose simulations indicated that the standard deviation of the home 
CPI inflation rate can be substantially reduced by joining a monetary union, can not be empirically validated.

For the countries in the core of the eurozone, one clearly visible trend is the rise in 
inflation volatility in almost half of the euro countries. \citet{emerson} emphasize  the fact that a high inflation rate is also more
...
The good news is that mean inflation declined considerably, most notably in the Eastern European countries that clearly 
benefited from the monetary discipline that is demanded for those countries who want to join the EMU.
