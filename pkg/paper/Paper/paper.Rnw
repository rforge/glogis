...
\usepackage{amsfonts,amstext,amsmath,thumbpdf}
...
\Abstract{
  ...
  with reduced skewness, but it is accompanied by an increase in variance.
}
...
\fancyhead[CO]{\textit{\normalsize Thomas Windberger, Achim Zeileis}}
\fancyhead[CE]{\textit{\normalsize Structural Breaks in Inflation Dynamics within the European Monetary Union}}
\fancyfoot[LO,LE]{\small Copyright {\copyright} 2014 M.E. Sharpe, Inc.}
\fancypagestyle{plain}{
\fancyhf{}
\fancyfoot[LO,LE]{\small This is a preprint of an article published in \textit{Eastern European Economics}, \textbf{52}(3), 67--89.
  \doi{10.2753/EEE0012-8775520304}.
  Copyright~{\copyright} 2014 M.E. Sharpe, Inc.
  }
}
...
a transition period ending with the replacement of the national currencies by the euro),
...
inflation rates attract the widest interest, but there are also reasons for investigating
...
causes more relative price variability, leading to a less
...
also of higher moments. Hence, we contribute to this field of research by
...
signaling a reduction of size and frequency of outlying high inflation rates.
...
All empirical analyses are based on seasonally adjusted monthly inflation rates (in percent)
...
%% and are also not members of the OECD
...
changes of the seasonal inflation patterns over time within several countries
...
can be assessed, and (4)~how breakpoints can be estimated in the presence of parameter
instability.
...
scores $s(y_i | \hat \phi)$ as measures of model deviation from the model fit under $H_0$
\citep[as in][]{Zeileis+Hornik:2007}. Systematic deviations over time from the full sample estimates $\hat \phi$
...
\end{eqnarray}
%
and the appropriate $p$~values can be computed from the corresponding
...
for(i in seq_along(nam)) {
  ...
  results[[i]] <- list(
    ...
    model0 = x_gf,
    efp = x_efp,
    efp_hac = gefp(x_gf, fit = NULL, vcov = vcovHAC), #FIXME# added in revision for EmpEc, should be moved to 'Analysis'
    nbreaks = x_nbreaks,
    breakpoints = x_bp,
    model = x_rf,
    erm = erm[i],
...
to stable growth in recent years. The good economic performance made it
...
lines(fitted(x$breakpoints, breaks = x$nbreaks, type = "mean"), lwd = 2)
if(x$nbreaks > 0) lines(confint(x$breakpoints, breaks = x$nbreaks), col = "darkgray")

plot(x$efp, functional = supLM(0.1), main = "", ylab = "LM statistics", ylim = c(0, 40), boundary = FALSE)
abline(h = supLM(0.1)$computeCritval(0.05, 3), col = "darkgray", lwd = 1.5)
lines(x$breakpoints, breaks = x$nbreakss)
@
\caption{\label{fig:slovenia1} 
  Inflation rate series for Slovenia with breakpoint estimate (\Sexpr{breakdates(x$breakpoints)})
  and associated sequence of LM statistics
  Left: Inflation rate series with breakpoint estimate (vertical dashed line),
  associated confidence interval (gray, at the bottom), and fitted mean from generalized
  logistic distribution (bold horizontal lines). Right: Corresponding sup$\mathit{LM}$ test
  with sequence of LM statistics, critical value at 5\% level (horizontal gray
  line), and estimated breakpoint (vertical dashed line).
}
...
for(i in 1:2) plot(x$model[[i]], moments = TRUE,
  ...
  xlab = paste(names(x$model)[i], "\n", "Goodness-of-fit p-value: ",
    ...
    sep = ""))
@
\caption{\label{fig:slovenia2} Histogram of observed monthly inflation rates in
Slovenia for \Sexpr{names(x$model)[1]} (left) and \Sexpr{names(x$model)[2]} (right),
along with fitted generalized logistic probability density function, associated
moments, and $\chi^2$ goodness-of-fit test.}
\end{figure}

Assuming a single stable set of parameters as in (\ref{eq:h0}) is  not valid, since the
...
$p$-value \Sexpr{spval[1]}. If a quadratic spectral kernel HAC estimator \citep{Andrews:1991}
...
associated with the breakpoint \Sexpr{breakdates(x$breakpoints, breaks = x$nbreaks)}, depicted
by a vertical dashed line in both panels of Figure~\ref{fig:slovenia1}. The resulting segmented
fitted mean (horizontal bold lines in the left panel of Figure~\ref{fig:slovenia1}) shows that
before the break, Slovenia experienced very high inflation rates but was successful in reducing
...
The success of these actions ensured the Slovenian participation in
the ERM~II only a year later and the introduction of the euro two years afterwards. 
...
<<results-diagnostics>>=
names(results)[names(results) == "CzechRepublic"] <- "Czech Rep."
format_diag <- function(i, digits = 3) {
  pround <- function(x) ifelse(abs(x) < 0.001, "$< 0.001$",
    paste("$", format(round(x, digits = digits), nsmall = digits), "$", sep = ""))
  
  rval <- cbind("", "", names(results[[i]]$model), "", "", "")
  n <- NROW(rval)
  if(n <= 1) rval <- rbind(rval, "")
  
  rval[1,   1] <- if(is.character(i)) i else names(results)[i]
  rval[1,   2] <- pround(sctest(results[[i]]$efp,     functional = supLM(0.1))$p.value)
  rval[2,   2] <- pround(sctest(results[[i]]$efp_hac, functional = supLM(0.1))$p.value)
  rval[1:n, 4] <- pround(sapply(results[[i]]$model, function(obj) summary(obj)$chisq.test$p.value))
  rval[1:n, 5] <- pround(sapply(results[[i]]$model, function(obj) acf(obj$x, plot = FALSE)$acf[2]))
  rval[1:n, 6] <- pround(sapply(results[[i]]$model, function(obj) Box.test(obj$x, type = "Ljung-Box")$p.value))
...
\hline
Country & sup$\mathit{LM}$ (standard/HAC) & Segment & $\chi^2$ GOF & AR(1) & Ljung-Box \\
\hline

\noalign{\medskip}
\multicolumn{6}{l}{\emph{No change}} \\
\hline
<<diagnostics-table1, results=tex>>=
writeLines(unlist(lapply(c(2, 4, 8, 14), format_diag)))
@

\noalign{\medskip}
\multicolumn{6}{l}{\emph{Phase~1 convergence}} \\
\hline
<<diagnostics-table2, results=tex>>=
writeLines(unlist(lapply(c(1, 6, 7), format_diag)))
@

\noalign{\medskip}
\multicolumn{6}{l}{\emph{Phase~2 convergence}} \\
\hline
<<diagnostics-table3, results=tex>>=
writeLines(unlist(lapply(c(9, 12, 13, 16), format_diag)))
@

\noalign{\medskip}
\multicolumn{6}{l}{\emph{Financial crisis}} \\
\hline
<<diagnostics-table4, results=tex>>=
writeLines(unlist(lapply(c(11, 19), format_diag)))
@

\noalign{\medskip}
\multicolumn{6}{l}{\emph{Non-euro}} \\
\hline
<<diagnostics-table5, results=tex>>=
writeLines(unlist(lapply(c(20, 21), format_diag)))
@

\noalign{\medskip}
\multicolumn{6}{l}{\emph{Eastern countries}} \\
\hline
<<diagnostics-table6, results=tex>>=
writeLines(unlist(lapply(c(3, 5, 10, 15, 17, 18), format_diag)))
@

\noalign{\medskip}
\multicolumn{6}{l}{\emph{Euro area}} \\
\hline
<<diagnostics-table6, results=tex>>=
writeLines(unlist(lapply(22, format_diag)))
@

\end{tabular}

\caption{\label{tab:diag} Diagnostics for model fits in each country. Two sup$\mathit{LM}$ test $p$~values
  of the full-sample model with and without HAC correction and segment-specific assessments of the model
  fit: $\chi^2$ goodness-of-fit (GOF) $p$~value; autocorrelation at lag~1; Ljung-Box $p$~value.}
\end{table}
...
structural breaks in the inflation dynamics within each country are
...
(breakpoints and segment-specific GL parameters) were estimated by ML. 
The $p$~values from the sup$\mathit{LM}$ test with and without HAC correction are reported in Table~\ref{tab:diag} 
along with further diagnostics for each of the fitted segments concerning goodness of fit and autocorrelation. 
As autocorrelation was very low in most of the segments (typically below 0.4 and non-significant, with a few exceptions), 
we focus on the results from the analysis without HAC correction but point out if there are noteworthy deviations. 
In both Table~\ref{tab:results1} and~\ref{tab:diag} the countries are grouped  according to either a
...
these countries were already much in line with Germany and hence the ECB. Table~\ref{tab:diag} does not 
suggest any problems in this group of countries.

%% source for DM zone: http://ec.europa.eu/economy_finance/publications/publication6730_de.pdf

The second group of countries, Austria, Finland, and France, adjusted to the requirements of the EMU during the
first phase of convergence. In these countries, the result of the adjustment process
was a lower inflation rate and an overall decrease in the variance.
Additionally, if a HAC correction is used, the change for France would be nonsignificant,
suggesting that France may also belong to the group of countries which did not change at all.
...
in terms of mean and variance, also associated with a shift in skewness towards the
...
rates, which are closer to the levels of the previous two groups. The diagnostics 
from Table~\ref{tab:diag} indicate good model fits with low autocorrelations.
...
so strongly.  The model fit diagnostics appear to be reasonable except for the middle
segment of Spain, where the Ljung-Box test shows that not all of the autocorrelation has been captured.

The fifth group consists of two countries that, after a very short participation in the
...
in ``Black Wednesday'' in 1992 and the banking crisis in Sweden during the early
1990s. With regard to the last segment for both countries, the Ljung-Box test suggests that 
the autocorrelation, albeit low around 0.16, is significant. However, both the standard
and the HAC-corrected sup$\mathit{LM}$ lead to highly significant evidence for structural
breaks.
...
Czech Republic, Estonia, and Hungary experienced a break in 1997/1998
resulting from efforts to curb in inflation by decreasing the growth of the money 
supply. The timing of the break thus indicates the date when the the biggest
transitionally shocks on the way towards free market economies have been overcome. In Poland, this
transition took two more years. Slovakia and Slovenia are somewhat different: On the one hand,
they started with lower mean inflation rates and on the other hand, their inflation dynamics
both changed roughly one year prior to their entry into the ERM~II. Thus,
compared to other inflation rates, these series show the most dynamic behaviour. However,
the models also do not fit as well as for the previous groups of countries.
There is the highest occurrence of non-captured autocorrelation and for Estonia
and Slovakia the $\chi^2$ goodness-of-fit test suggests that the generalized logistic distribution
might not fit sufficiently well in the first segments.
...
EU17 series as provided by the \citet{oecd},
starting in \Sexpr{start(eu) - 1/12}. It is calculated as a weighted average of the HICPs
of the then seventeen euro countries.} are considered and
...
the breakpoints estimation results) as the ``core'' of the euro zone, the PIIGS countries,
...
slightly (in both quartiles) and skewness decreased somewhat (from slightly
right- to slightly left-skewed). The heterogeneity (assessed by the
interquartile range) declined for all three moments of the inflation rates.
...
For the PIIGS countries, changes occurred from \Sexpr{yrs[1]} to
...
symmetric. The latter reflects that the occurrence of shocks with high inflation
...
several extremely low inflation rates during the time
...
to \Sexpr{yrs[3]}, encompassing both the completion of the transformation from
...
mean inflation is in between the levels for the core euro and PIIGS countries,
variance is still slightly higher, and skewness is not too far away from symmetry.
...
has been reduced for all three groups of countries but to a different extent.
...
gray_red <- rgb(c(0.2, 0.8), c(0.2, 0.0), c(0.2, 0.0), alpha = 0.3)
gray_red <- rgb(c(0.2, 0.0), c(0.2, 0.0), c(0.2, 0.0), alpha = c(0.2, 0.5)) ## actually gray-darkgray now
...
@
\caption{\label{fig:glmean} Observed monthly inflation rates (light gray) and estimated means from
12-month rolling GL fits (dark red) over time for all countries.}
\end{figure}
\setkeys{Gin}{width=\textwidth}
...
This may be somewhat surprising and is not entirely correct. In fact,
...
One way to bring out this pattern graphically is
...
convergence hypotheses, since the first three moments from all countries in our sample
...
\proglang{R}~3.0.0 %\Sexpr{paste(R.Version()[6:7], collapse = ".")}
\citep{R} with the packages
\pkg{glogis}~0.1-0, %\Sexpr{packageDescription("glogis")["Version"]},
\pkg{strucchange}~1.4-7 %\Sexpr{packageDescription("strucchange")["Version"]}
\citep{Zeileis+Leisch+Hornik:2002}, and
\pkg{fxregime}~1.0-2 %\Sexpr{packageDescription("fxregime")["Version"]}
\citep{Zeileis+Shah+Patnaik:2010},
...
the raw monthly price indexes \citep[as provided by the][]{oecd} and the seasonally adjusted
...
via\linebreak \code{demo("Austria", package = "glogis")} etc.
