...
\usepackage{graphicx,a4wide,color,hyperref}
...
% vielleicht ein paar Länder aus dem Sample entfernen: 
% slovakei, tschechien, ungarn, polen
% Nachschauen, wann in den jeweiligen Ländern die Entscheidung getroffen wurde, den Euro einzuführen. --> viel zu aufwendig
...
\begin{eqnarray}
l(\delta,\theta,\sigma, \pi) & = & \log(\delta) - \log(\sigma) - \frac{1}{\sigma} (\pi-\theta) - (\delta+1) \log(1+\exp^{-\frac{\pi-\theta}{\sigma}})
\end{eqnarray}

The resulting score function ($\psi()$) for the parameters (the derivatives of the log-likelihood) are given by:

\begin{eqnarray}
\psi(\pi_i,\delta) & = & \frac{\delta l(\delta,\theta,\sigma;\pi)}{\delta \delta} = \frac{1}{\delta} - \log(1+\exp^{-\frac{\pi-\theta}{\sigma}}) \\
...
We use the Cramér von Mises type test as given in \cite{z07}. The test statistic is given by:
...
%% information from: Heiberger/Holland: Statistical Analysis and Data Display
%
%The one sample test tests whether a random sample is coming from a hypothesized null distribution. It compares the maximum discrepancy between the empirical cumulative distribution of the data, defined as S(y) = fraction of the data that is less than or equal to y and the cdf of the hypothesized population being sampled. The null hypothesis (ie. that a dataset has a distribution described by a fully determined (theoretical) distribution function) is rejected at a given significance level if the testing statistic exceeds the critical value.
%It is not possible to use the Kolmogorov–Smirnov test when the parameters of the distribution function are estimated from the same dataset onwhich they are tested. This, however, is usually the case in any practical application. This approach leads to type II error (failing to reject the null hypothesiswhen it is false).
%% Bemerkung: der ganze obige Absatz stammt aus: gamma-ksmirnov im Goodness-of-Fit Ordner; daher etwas umformulieren
%
%% this text: Kendal's Advanced Theory of Statistics, Sixth Edition, Volume 2A, by Alan Stuard et al.
%The measure of deviation used is the maximum difference between $S_n(x)$ (the cdf given by the observed sample) and $F_0(x)$ (the cdf hypothesized under $H_0$). Where $S_n(x)$ is r/n where r is the number of observations less than or equal x and n is the sample size.
%
%\begin{eqnarray}
%& & D_n = \underset{x}{sup}|S_n(x)-F_0(x)|
%\end{eqnarray}
%
%When the hypothesized distribution is composite, say $F_0(x|\theta_1,\theta_2,...\theta_s)$, with s paramters $\theta_j$, we must select s functions $t_1,...t_s$ of the $x_i$ for this purpose. [in the easy case $F_0$ is uniformly distributed on (0,1)] The transformed variables are now:
%
%\begin{eqnarray}
%& & y_i = \int_{-\infty}^{x_i} f_0(t_1,t_2,..t_s) du
%\end{eqnarray}
%
%but they are neither independent nor uniformly distributed, and their disribution depend on general both on the hypothetical distribution $F_0$ and on the true values of its parameters. So in our 3--parameter case there is some problem. 
...
% 
%% sup ist hier das Maximum
%% ist der Wert grösser als der kritische Wert, wird die H0 abgelehnt
%% http://de.wikipedia.org/wiki/Kolmogorow-Smirnow-Test
%
%
%% There is some more refined distribution theory for the KS test with estimated parameters (see Durbin, 1973), but that is not implemented in ks.test.
...
%
%http://www.sciencedirect.com/science?_ob=ArticleURL&_udi=B6V47-4D452FR-4P&_user=10&_coverDate=12%2F31%2F1982&_rdoc=1&_fmt=high&_orig=search&_sort=d&_docanchor=&view=c&_searchStrId=1385134404&_rerunOrigin=google&_acct=C000050221&_version=1&_urlVersion=0&_userid=10&md5=3102a52af9d8937550065045f0b94ee3
%
%
%
%%Handbook of parametric and nonparametric statistical procedures von Sheskin David wäre nützlich
%
%
%\subsubsection{The $\chi^2$ Test}
%
%The main problem is to determine the number of classes k (data is grouped into k classes where we then calculate the difference between observed and expected frequencies). In a continouus distribution case, there are no natural boundaries. One procedure advocated by Mann and Gumbel was to choose the classes so that - given k - the hypothetical probabilites are all equal to 1/k. Recommendations: use classes with equal or nearly equal probabilities. Determine number of classes by:
%
%\begin{eqnarray}
%& & k = b (\frac{2^{1/2}(n-1)}{\lambda_{\alpha} + G^{-1}P_0})
%\end{eqnarray}
%
%with b between 2 and 4; where $\lambda_{\alpha}$ is the critical value (ie. 1.64 in normal distribution case); P is the value of the power function, and $G^{-1}$ comes from ????.
%In case of ML estimation, critical values should be adjusted upwards.
...
%
%\begin{eqnarray}
%& & \chi^2 = \sum_{i=1}^k \frac{O_i - E_i}{E_i}
%\end{eqnarray}
%
%where the $O_i$ are the observed frequencies and the $E_i$ the expected frequencies. Null hypothesis is rejected if: $\chi^2 $>$ \chi^2_{\alpha, k-c}$.
%One rule would be to let k be equal to  $3.765(N-1)^{2/5}$ for a 5\% significance level, when using intervals with equal probability under $H_0$. But the statistical community still does not agree on the optimal choice of k. [see 15715341- goodness of fit folder, untere und obere grenze sind +- infinity]
%
%
%
%
...
%- Euro Währung Buchgeld seit 1.1.1999
%- Euro Währung Bargeld seit 1.1.2002?
...
\begin{tabular}{llll}
\hline
Country        & Type   & \multicolumn{2}{l}{Breakpoints}  \\ \hline
Austria        & EURO   & Sep 2007 &          \\
Belgium        & EURO   & Dec 1999 &          \\
Czech Republic &        & Jul 1998 &          \\
Denmark        & ERM~II & Jun 2000 &          \\
Estonia        &        & TODO     &          \\
Finland        & EURO   & none     &          \\
France         & EURO   & Dec 2004 &          \\
Germany        & EURO   & May 2000 & Dec 2004 \\
Greece         & EURO   & none     &          \\
Hungary        &        & May 1998 &          \\
Ireland        & EURO   & Mar 2008 &          \\
Italy          & EURO   & May 1996 & Dec 2000 \\
Luxembourg     & EURO   & Dec 1998 &          \\
Netherlands    & EURO   & none     &          \\
Poland         &        & May 2001 &          \\
Portugal       & EURO   & Jul 1992 & Mar 2004 \\
Slovakia       & EURO   & Apr 1997 & Feb 2004 \\
Slovenia       & EURO   & Jul 2003 &          \\
Spain          & EURO   & May 1996 & Dec 2000 \\
Sweden         &        & Jan 1993 &          \\
United Kingdom &        & Apr 1992 &          \\ \hline
\end{tabular}
\caption{\label{tab:breakpoints} Caption needed.}
\end{center}
\end{table}
...
%
%falls Volatilität erhöht wird, verweise auf die Debatte in: $2004-wp-minford-ukjoiningemu$, die von einer Erhöhung der britischen Inflationsrate sprechen, sollte UK der EMU beitreten.
%
%falls kleine Länder auffälliges Verhalten zeigen: $2008-wp-arnold-inflationuncertainty$, "`This suggests that within EMU, inflation uncertainty may increase in countries that have a smaller influence on ECB policy."' Ihre Begründung: "`For economic agents in small regions which are hit by positive inflation shocks, it will be uncertain how and when the increase
...
%
%berücksichtige die DM Zone (Deutschland, Österreich, Luxemburg, Belgien und Holland), eventuell ähnlich
%
...
%
%Wenns grosse Unterschiede zwischen den EMU Ländern gibt: ba Caporale stoht: "`the ECb itself admits that monetary policy can only affect price developements in the Euro area as a whole and cannot influence inflation differentials across regions (The Monetary Policy of the ECB, 2004)."' Und dort findet sich auch: "`some short-term volatility is inevitable given the fact that monetary policy can only affect prices wiht long and uncertain lags, hence the focus of the ECB on medium-term price stabilisation."'
%
%Um den Anstieg in der Volatilität zu verteidigen: Caporale finds that steady state uncertainty has increased in eight out of twelve countries (p.966) (ie. Germany, France, Italy, Spain, Ireland, Finland, Luxembourg and Austria).
%
%Zum Abgleich für Break dates: Caporale Table 5.
%
%
%Wenn Ergebnisse da sind: abgleichen bei Caporale mit seiner Fig. 2, Fig. 3
%
%falls Österreich und Spanien komisch: Wilfing findet, dass Schilling und Peseta nicht zu ihrer fixen Parität konvergiert sind
%
%falls man grosse Unterschiede in der Volatilität sehen würde: $2008-wp-campolmi-laborinflvol$ point out that this can be explained by country specific labor market institutions.
%
% morana paper: S.10 untersucht convergence, je nachdem, was sich findet herausfinden, was wie ähnlich ist
%
...
\subsection{Austria}
%% the information is saved in a folder called "`Erläuterungen"' which has the country name
...
% Country Report Belgium  bei ebscohost in TX all text
There was a  very sharp fall in yearly inflation in this time (from 2\% in December 1999 to 0.2\% in January 2000). Volatility  a very clear increase in volatility; from 1999 to 2000 we had a 137\% increase in the yearly inflation rate. Economic reason: belgish franc was pegged to German Mark (since 1990). Possibly: Domestic inflation was mainly driven by the surge in world oil prices, with the result that energy prices in Belgium were around 25\% higher in the first quarter of this year than in the year-earlier period. Also a general upward trend in prices.
% source: Belgium_3206697
tax reductions in personal income tax
% source: Belgium_4611003
inflation turns out to be more volatile in small, open economies such as Belgium (might be the case in some countries, but not in all of them). some explanations happen to be: a) a more volatile currency risk premium (which cannot be the case in Belgium, since it happend after 1.1999 and the currency was pegged thereafter, and even before pegged to Germany); b) degree of exchange--rate--passthrough to inflation (not possible as well) and c) size of monetary policy shocks (if there were any, should have to be observed in ALL EMU countries, since monetary policy equal). another possible explanation are productivity shocks which boost inflation rate volatility (but this for more than 10 years????)


[some connection with interest rate? maybe not the same until 1.2002 ?]
[volatility of inflation decreases the rate of growth]
%(growth_rate_vol_inflation)
...
\subsection{Ireland}
In recession since second quarter of 2008. The European country most severely hit by the recession
...
Economic reasons: in November 1996 Italy rejoined the european monetary system (left it in 1992); in September 1996, the new Cabinet announced Italy's decision to take part in the Monetary Union. After 2000 Italy entered an econmic crisis [auf Auffälligkeiten eingehen]
...
%- definition of the distribution Abberger uses here is a generalization of 23.7 in Johnson 95 (shape ist halt hier zustätzlich noch dabei)
...
%\subsection{Austria}
%
%The Double max test for Austria is insignificant, but the Cramér-von Mises test and the Supremum of LM statistcs test are both significant. This can be interpreted as a sign, that there was some change in the series as a whole, but that we are unsure wheter this happend with respect to location, scale or shape.
%
%\begin{figure}[h!tp]
%\includegraphics[width=80mm]{aut-meanL2BB.png}
%\caption{Cramér-von Mises test for Austria}
%\end{figure}
%
...
%\caption{Cramér-von Mises test for Belgium}
%\end{figure}
%
...
%%so if it is the case like 6-2-1 we can say that the distribution gets less of a positive skew meaning a lower inflation on average (abklären, sobald das mit den Momenten hinhaut)
%%
%
%\subsection{Italy}
%
...
%\caption{Cramér-von Mises test for Italy}
%\end{figure}
%
