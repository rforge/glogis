...
## <<tabelle, eval=FALSE>>=
if(!file.exists(file.path("C:/Dissertation/Paper-1-Z", "Analysis", "Austria.rda"))) {
setwd(file.path("..", "Analysis"))
Sweave("analysis.Rnw")
setwd(file.path("..", "Slides"))
}
...
<<bla, eval=TRUE>>=
setwd("C:/Dissertation/Paper-1-Z/R-Forge2/pkg/paper/Analysis")
load("Belgium.Rda")
xbel <- x
xbel_bp <- x_bp
xbel_efp <- x_efp
xbel_gf <- x_gf
xbelnbreaks <- x_nbreaks
xbel_rf <- x_rf
load("Luxembourg.Rda")
...
plot(xbel, main = "Series with Fitted Mean", ylab = "100 * diff(log(HICP))", xlab = "Time")
lines(xbel_bp, breaks = xbelnbreaks)
lines(fitted(xbel_bp, breaks = xbelnbreaks, type = "mean"), col = 4)
@
...
<<plot1d, eval=FALSE>>=
plot(x, main = "Slovenia", ylab = "100 * diff(log(HICP))", xlab = "Time")
@
...
<<plot4, eval=FALSE>>=
par(mfrow = c(1, 1))
plot(x_efp, aggregate = FALSE, ylim=c(-2.1,2.1))
@
...
% to show both series with breakpoints and means
% does not work
%\begin{center}
%<<belluxvers, fig=TRUE, height=7, width=8>>=
%<<plotversuch>>
%@
%\end{center}


On the basis of these results, we are able to trace out different groups. The first of this group consits of Belgium and Luxembourg. This may come as no surprise since both countries formed a monetary union prior even to the start of the European Rate Mechanism (ERM). While both countries enjoyed very stable inflation rates with low values of both mean and variance, this changed around the timing of the irrevocable fixing of the exchange rates. A causal connection to this events seems likely.
...
\hline
<<echo=FALSE, results=tex>>=
writeLines(unlist(lapply(c(5,10,15,17,18), format_moments)))
@
\end{longtable}

\begin{center}
<<eastern, fig=TRUE, height=7, width=8>>=
print(xyplot(cbind(100*diff(log(na.omit(HICP[, "CzechRepublic"]))), 100*diff(log(na.omit(HICP[, "Estonia"]))), 100*diff(log(na.omit(HICP[, "Hungary"]))), 100*diff(log(na.omit(HICP[, "Poland"]))), 100*diff(log(na.omit(HICP[, "Slovakia"]))), 100*diff(log(na.omit(HICP[, "Slovenia"])))), xlab="Time", ylab="", main="", screens=list("Czech Republic", "Estonia", "Hungary", "Poland", "Slovakia", "Slovenia")))
@
\end{center}
...
% could be looking much better
\begin{center}
<<itaspa, fig=TRUE, height=7, width=8>>=
...
\begin{center}
<<ireland, fig=TRUE, height=7, width=8>>=
...
\begin{center}
<<nochange, fig=TRUE, height=7, width=8>>=
...
No clear message is available for: Denmark, France, Germany and Portugal. 
...
@
\begin{center}
<<autplot, fig=TRUE, height=4, width=8>>=
<<plot1>>
@
\end{center}
