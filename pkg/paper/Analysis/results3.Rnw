...
if (i == 12) {x_nbreaks <- 2} else {x_nbreaks <- 1}
...
%-----------------------------------------------------------
\newpage
% Greece (Jan,Feb, March, Jul,Aug, Sept)
<<>>=
i <- 9
nam <- colnames(HICP)[i]
fnam <- paste(nam, ".rda", sep = "")
...
%-----------------------------------------------------------
\newpage
% Italy ((Jan, March, Jul, Sept ?) --> not clear
<<>>=
i <- 12
nam <- colnames(HICP)[i]
fnam <- paste(nam, ".rda", sep = "")
...
for(k in 1:length(mon)) {
if ( mon[k] == 3) {subs[k] <- reg$coefficients[3]} else {subs1[k] <- 0} 
}
...
for(k in 1:length(mon)) {
if ( mon[k] == 9) {subs[k] <- reg$coefficients[5]} else {subs1[k] <- 0} 
}
