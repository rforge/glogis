...
add_breaks[c("Germany", "Portugal")] <- 1
@
...
%% generate monthly dummies for period AFTER inclusion of sales 
...
#### values for dummie regression of OLD values
%% generate monthly dummies for period PRIOR to the inclusion of sales
mona <- rep(seq(1,12),30)
mona <- mona[1:n2]

dum1a <- seq(n1, length(mona))
dum2a <- seq(n1, length(mona))
dum3a <- seq(n1, length(mona))
dum7a <- seq(n1, length(mona))
dum8a <- seq(n1, length(mona))
dum9a <- seq(n1, length(mona))

# loops for dummy creation
for(j in 1:length(mona)) {
if ( mona[j] == 1) {dum1a[j] <- 1} else {dum1a[j] <- 0} 
}

for(j in 1:length(mona)) {
if ( mona[j] == 2) {dum2a[j] <- 1} else {dum2a[j] <- 0} 
}

for(j in 1:length(mona)) {
if ( mona[j] == 3) {dum3a[j] <- 1} else {dum3a[j] <- 0} 
}


for(j in 1:length(mona)) {
if ( mona[j] == 7) {dum7a[j] <- 1} else {dum7a[j] <- 0} 
}


for(j in 1:length(mona)) {
if ( mona[j] == 8) {dum8a[j] <- 1} else {dum8a[j] <- 0} 
}


for(j in 1:length(mona)) {
if ( mona[j] == 9) {dum9a[j] <- 1} else {dum9a[j] <- 0} 
}
...
## had to be changed due to error 
...
# substract difference between old dummies and new dummies
	regalt <- lm(x[1:n2] ~ dum1a + dum2a + dum7a + dum8a)
...
if ( mon[k] == 1) {subs[k] <- reg$coefficients[2]-regalt$coefficients[2]} else {subs1[k] <- 0} 
}


for(k in 1:length(mon)) {
if ( mon[k] == 2) {subs[k] <- reg$coefficients[3]-regalt$coefficients[3]} else {subs1[k] <- 0} 
}

for(k in 1:length(mon)) {
if ( mon[k] == 7) {subs[k] <- reg$coefficients[4]-regalt$coefficients[4]} else {subs1[k] <- 0} 
}


for(k in 1:length(mon)) {
if ( mon[k] == 8) {subs[k] <- reg$coefficients[5]-regalt$coefficients[5]} else {subs1[k] <- 0} 
}
...
# regress on monthly dummies
  regalt <- lm(x[1:n2] ~ dum1a + dum2a + dum3a +  dum7a  + dum8a + dum9a)
...
for(k in 1:length(mon)) {
if ( mon[k] == 3) {subs[k] <- reg$coefficients[4]-regalt$coefficients[4]} else {subs1[k] <- 0} 
}


for(k in 1:length(mon)) {
if ( mon[k] == 7) {subs[k] <- reg$coefficients[5]-regalt$coefficients[5]} else {subs1[k] <- 0} 
}


for(k in 1:length(mon)) {
if ( mon[k] == 8) {subs[k] <- reg$coefficients[6]-regalt$coefficients[6]} else {subs1[k] <- 0} 
}

for(k in 1:length(mon)) {
if ( mon[k] == 9) {subs[k] <- reg$coefficients[7]-regalt$coefficients[7]} else {subs1[k] <- 0} 
}
<<analysis>>
}
@
